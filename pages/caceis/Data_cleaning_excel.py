...
def clear_data(df : pd.DataFrame):
    df = df[["Libellé compte cash","Devise compte","Date comptable","Montant mouvement (devise compte)","Description mouvement"]]
    ...
    for row in df.itertuples(index = False):
        new_row_1 = {
            ...
            "N° Piéce": "",
            "Libellé": row[-1],     # Description mouvement
    ...
    new_df = pd.DataFrame(rows, columns=["Date","journal","Compte","N° Piéce", "Libellé", "Débit", "Crédit", "Monnaie"])
